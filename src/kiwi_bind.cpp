...
inline kiwi::POSTag parseTag(const char* tag){
  auto u16 = kiwi::utf8To16(tag);
  transform(u16.begin(), u16.end(), u16.begin(), static_cast<int(*)(int)>(toupper));
  auto pos = kiwi::toPOSTag(u16);
  if (pos >= kiwi::POSTag::max) throw std::invalid_argument{ std::string{"Unknown tag value : "} + tag };
  return pos;
}
...
// [[cpp11::register]]
int kiwi_builder_add_pre_analyzed_word_(SEXP handle_ex, const std::string form, const cpp11::data_frame analyzed_r, float score) {
  cpp11::external_pointer<kiwi_builder> handle(handle_ex);
  ...
  char** analyzed_morphs;
  char** analyzed_pos;
  int* positions;

  for (int i = 0; i < size; ++i) {
    strcpy(analyzed_morphs[i], std::string(morphs[i]).c_str());
    strcpy(analyzed_pos[i], std::string(pos[i]).c_str());
    positions[i * 2] = start[i];
    positions[i * 2 + 1] = end[i];
  }

  // return kiwi_builder_add_pre_analyzed_word(handle.get(), form.c_str(), size, analyzed_morphs, analyzed_pos, score, positions);
  return 1;
}
