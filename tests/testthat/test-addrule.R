...
test_that("add rule works", {
  ...
  tryCatch({
    ...
    kw_baseline_builder <- elbird:::kiwi_builder_init_(elbird:::kiwi_model_path_full("base"), 0, elbird:::kiwi_default_build_options())
    if(is.null(kw_baseline_builder)) stop("Failed to init baseline kiwi_builder_init_ for base model.")
    kw_orig_ptr <- elbird:::kiwi_builder_build_(kw_baseline_builder)
    if(is.null(kw_orig_ptr)) stop("Failed to build baseline kiwi_builder_build_ for base model.")

    text1 <- "했어요! 하잖아요! 할까요?"
    res1 <- elbird:::kiwi_analyze_wrap(kw_orig_ptr, text1, top_n = 1)
    ...
    res2_orig <- elbird:::kiwi_analyze_wrap(kw_orig_ptr, text2, top_n = 1)
    ...
    kb_rule_builder_ptr <- elbird:::kiwi_builder_init_(elbird:::kiwi_model_path_full("base"), 0, elbird:::kiwi_default_build_options())
    if (is.null(kb_rule_builder_ptr)) {
      stop("Failed to initialize kiwi_builder for rule with base model.")
    }

    add_rule_result <- elbird:::kiwi_builder_add_rule_(kb_rule_builder_ptr, "EF", "요$", "용", -2.0)
    if (add_rule_result < 0L) {
      stop(paste0("kiwi_builder_add_rule_ failed with code: ", add_rule_result))
    }

    kw_with_rule_ptr <- elbird:::kiwi_builder_build_(kb_rule_builder_ptr)
    if (is.null(kw_with_rule_ptr)) {
      stop("Failed to build kiwi from builder (with rule, base model).")
    }

    res_with_rule <- elbird:::kiwi_analyze_wrap(kw_with_rule_ptr, text2, top_n = 1)
